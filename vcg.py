...
class VCG:
    ...
    def compute(slot_clicks, reserve, bids):
        ...
        def total_payment(k):
            ...
            #Since invalid (below the reserve) bids have been filtered,
            #we don't need to account for that here.

            #If you are the last bidder allocated, you pay
            #slot_click*max of reserve and your bid
            if k == (n-1):
                return c[k]*max(reserve, valid_bids[k][1])

            #Otherwise, use the recursive payment structure to calculate
            #the payment
            else:
                return (c[k]-c[k+1])*valid_bids[k][1]+total_payment(k+1)
